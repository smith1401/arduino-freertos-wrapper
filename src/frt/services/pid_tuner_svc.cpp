...
PIDTunerServiceTCP::PIDTunerServiceTCP(const char *ssid, const char *password, uint16_t port)
{
...
bool PIDTunerServiceTCP::run()
{
    ...
    if (_input_sub->receive(input) && _client != nullptr)
    {
        const char *p = reinterpret_cast<const char *>(&input.temperature);
        _client->write(p);
    }
